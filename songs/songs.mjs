import aghArdeshirBadAzBadLyrics from "./aghArdeshir - Ba'd Az Bad/ba'd-az-bad.lyrics.mjs";
import nishkhandSarbasteyeYekSarbazLyrics from "./Nishkhand - Sarbasteye Yek Sarbaz/sarbasteye-yek-sarbaz.lyrics.mjs";

export const songs = {
  aghArdeshir: {
    badAzBad: {
      coverUrl:
        "songs/aghArdeshir - Ba'd Az Bad/ba'd-az-bad.original.1800x1800.png",
      songUrl: "songs/aghArdeshir - Ba'd Az Bad/aghArdeshir - Ba'd Az Bad.mp3",
      lyrics: aghArdeshirBadAzBadLyrics,
      bgColor: "#252527",
      textColor: "white",
      title: "بعد از بد",
      artist: "آق‌اردشیر",
    },
  },
  nishkhand: {
    sarbasteyeYekSarbaz: {
      coverUrl:
        "songs/Nishkhand - Sarbasteye Yek Sarbaz/sarbasteye-yek-sarbaz.cover.1200x1200.png",
      songUrl:
        "songs/Nishkhand - Sarbasteye Yek Sarbaz/Nishkhand - Sarbasteye Yek Sarbaz 128.mp3",
      lyrics: nishkhandSarbasteyeYekSarbazLyrics,
      bgColor: "white",
      textColor: "black",
      title: "سربسته‌ی یک سرباز",
      artist: "نیشخند",
    },
  },
};
